...
def extract_data_with_regex(text):
    """
    Parcourt tous les extracteurs disponibles et renvoie un dict
    avec *tous* les modèles détectés (requisition, po, piece, tool, profile, customer).
    """
    parsers = [
        ("requisition", extract_requisition),
        ("po", extract_po),
        ("piece", extract_piece),
        ("tool", extract_tool),
    ...
    for label, parser in parsers:
        # On appelle l'extracteur
        data = parser(text)
        # On conserve uniquement si au moins un champ non-null/nan est extrait
        if isinstance(data, dict) and any(v not in (None, "") for v in data.values()):
            results[label] = data

    return results  # vide {} si aucun modèle trouvé

def extract_requisition(text):
    requisition = {}
    requisition["requisitionStatus"] = _search(text, r"(requisition status|statut)\s*[:\-]?\s*(\w+)", 2)
    ...
    requisition["customerPurchaseNumber"] = _search(text, r"(PO Number|purchase number)\s*[:\-]?\s*(\w+)", 2)
    requisition["contact"]           = _search(text, r"(contact|die maker)\s*[:\-]?\s*(\w+)", 2)
    requisition["toolNumber"]        = _search(text, r"(tool number)\s*[:\-]?\s*(\w+)", 2)
    requisition["doubleLayout"]      = _bool("double layout" in text.lower())
    return requisition

def extract_po(text):
    po = {}
    po["poNumber"]   = _search(text, r"(PO\s*Number|Num[ée]ro de bon)\s*[:\-]?\s*(\S+)", 2)
    ...
    if dims:
        po["dimensions"] = {"width": float(dims.group(1)), "height": float(dims.group(2))}
